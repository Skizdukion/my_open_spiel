...
ABSL_FLAG(int, num_rows, 6, "Rows of gomuko");
ABSL_FLAG(int, num_cols, 6, "Rows of gomuko");
ABSL_FLAG(int, win_size, 4, "Rows of gomuko");
...
int main(int argc, char **argv) {
  ...
  if (positional_args.size() > 1) {
    ...
    open_spiel::json::Object config_json =
        open_spiel::json::FromString(config_string).value().GetObject();
  ...
  params["rows"] = open_spiel::GameParameter(absl::GetFlag(FLAGS_num_rows));
  params["cols"] = open_spiel::GameParameter(absl::GetFlag(FLAGS_num_cols));
  params["winSize"] = open_spiel::GameParameter(absl::GetFlag(FLAGS_win_size));

  std::shared_ptr<const open_spiel::Game> game =
      open_spiel::LoadGame("gomuko", params);

  return !open_spiel::algorithms::torch_az::AlphaZero(config, game, &stop_token,
                                                      resuming);
}