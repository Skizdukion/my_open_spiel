...
MCTSBot::MCTSBot(const Game &game, std::shared_ptr<Evaluator> evaluator,
  ...
  small_simulations_ = 0;
  small_simulations_sample_chances_ = 0;
}

MCTSBot::MCTSBot(const Game &game, std::shared_ptr<Evaluator> evaluator,
                 double uct_c, int max_simulations, int small_simulations,
                 int small_simulations_sample_chances, int64_t max_memory_mb,
                 bool solve, int seed, bool verbose,
                 ChildSelectionPolicy child_selection_policy,
                 double dirichlet_alpha, double dirichlet_epsilon,
                 bool dont_return_chance_node)
    : uct_c_{uct_c}, max_simulations_{max_simulations},
      small_simulations_{small_simulations},
      small_simulations_sample_chances_{small_simulations_sample_chances},
      max_nodes_((max_memory_mb << 20) / sizeof(SearchNode) + 1), nodes_(0),
      gc_limit_(MIN_GC_LIMIT), verbose_(verbose), solve_(solve),
      max_utility_(game.MaxUtility()), dirichlet_alpha_(dirichlet_alpha),
      dirichlet_epsilon_(dirichlet_epsilon),
  ...
  SPIEL_CHECK_LT(small_simulations_, max_simulations_);
  SPIEL_CHECK_GE(small_simulations_sample_chances_, 0.0);
  SPIEL_CHECK_LE(small_simulations_sample_chances_, 1.0);
...
Action MCTSBot::Step(const State &state) {
  ...
  if (max_simulations_ <= 1) {
    // sample from prior -> pure policy play (prior) without searching
...
std::unique_ptr<SearchNode> MCTSBot::MCTSearch(const State &state) {
  ...
  int num_simulation = max_simulations_;

  if (small_simulations_sample_chances_ > 0) {
    std::mt19937 rng(std::random_device{}());
    std::uniform_real_distribution<double> dist(0, 1);
    if (dist(rng) < small_simulations_sample_chances_) {
      num_simulation = small_simulations_;
      root->is_full_search = false;
    }
  }
